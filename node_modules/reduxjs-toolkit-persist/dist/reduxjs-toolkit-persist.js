(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.ReduxJSToolkitPersist = {}));
  ...
  var DISPATCH_EVENT = !!(document && document.createEvent && global$2.dispatchEvent);
  ...
  $$d({ global: true, wrap: true, forced: FORCED$2 }, {
  ...
  // statics
  $$d({ target: PROMISE, stat: true, forced: FORCED$2 }, {
  ...
  $$d({ target: PROMISE, stat: true, forced: IS_PURE || FORCED$2 }, {
  ...
  $$a({ target: 'Array', proto: true, forced: ES3_STRINGS || !STRICT_METHOD$4 }, {
  ...
  */
  function autoMergeLevel1(inboundState, originalState, reducedState, _ref) {
    ...
    if (inboundState && _typeof(inboundState) === 'object') {
      Object.keys(inboundState).forEach(function (key) {
        ...
        if (originalState[key] !== reducedState[key]) {
          if (debug) console.log('reduxjs-toolkit-persist/stateReconciler: sub state for key `%s` modified, skipping.', key);
          return;
    ...
    if (debug && inboundState && _typeof(inboundState) === 'object') console.log("reduxjs-toolkit-persist/stateReconciler: rehydrated keys '".concat(Object.keys(inboundState).join(', '), "'"));
    return newState;
  }
  ...
  $$8({ target: 'Array', proto: true, forced: NEGATIVE_ZERO || !STRICT_METHOD$3 }, {
  ...
  $$7({ target: 'Array', proto: true, forced: !STRICT_METHOD$2 || CHROME_BUG$1 }, {
    ...
    return storage.getItem(storageKey).then(function (serialized) {
      if (!serialized) return undefined;else {
        try {
          ...
          if (debug) console.log("reduxjs-toolkit-persist/getStoredState: Error restoring data ".concat(serialized), err);
          throw err;
        }
      }
    });
  ...
  function persistReducer(config, baseReducer) {
    ...
    return function (state, action) {
      var _ref = state || {},
          ...
          rest = _objectWithoutProperties(_ref, _excluded); // $FlowIgnore need to update State type
      ...
      if (action.type === PERSIST) {
        ...
        var _rehydrate = function _rehydrate(payload, err) {
          ...
          if (_sealed) console.error("reduxjs-toolkit-persist: rehydrate for \"".concat(config.key, "\" called after timeout."), payload, err); // only rehydrate if we are not already sealed
        ...
        getStoredState$1(config).then(function (restoredState) {
          ...
          migrate(restoredState, version).then(function (migratedState) {
            ...
            if (migrateErr) console.error('reduxjs-toolkit-persist: migration error', migrateErr);
  ...
  */
  ...
  $$4({ target: 'Array', proto: true, forced: !HAS_SPECIES_SUPPORT$2 }, {
  ...
  function persistStore(store, options, cb) {
    // help catch incorrect usage of passing PersistConfig in as PersistorOptions
    {
      ...
      bannedKeys.forEach(function (k) {
        if (!!optionsToTest[k]) console.error("reduxjs-toolkit-persist: invalid option passed to persistStore: \"".concat(k, "\". You may be incorrectly passing persistConfig into persistStore, whereas it should be passed into persistReducer."));
      });
    }
  ...
  function createMigrate(migrations, config) {
    ...
    return function (state, currentVersion) {
      if (!state) {
        if (debug) console.log('reduxjs-toolkit-persist: no inbound state, skipping migration');
      ...
      if (inboundVersion === currentVersion) {
        if (debug) console.log('reduxjs-toolkit-persist: versions match, noop migration');
      ...
      if (debug) console.log('reduxjs-toolkit-persist: migrationKeys', migrationKeys);

      try {
        var migratedState = migrationKeys.reduce(function (state, versionKey) {
          if ("development" !== 'production' && debug) console.log('reduxjs-toolkit-persist: running migration for versionKey', versionKey);
...
//# sourceMappingURL=reduxjs-toolkit-persist.js.map
