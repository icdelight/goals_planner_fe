...
function hasLocalStorage() {
  ...
  try {
    ...
    var testKey = "reduxjs-toolkit-persist localStorage test";
    storage.setItem(testKey, 'test');
    storage.getItem(testKey);
    storage.removeItem(testKey);
  } catch (e) {
    if (process.env.NODE_ENV !== 'production') console.warn("reduxjs-toolkit-persist localStorage test failed, persistence will be disabled.");
    return false;
  }
...
function getStoredStateV4(v4Config) {
  return new Promise(function (resolve, reject) {
    ...
    if (storage.keys && !storage.getAllKeys) storage = _objectSpread(_objectSpread({}, storage), {}, {
    ...
    storage.getAllKeys(function (err) {
      ...
      if (err) {
        if (process.env.NODE_ENV !== 'production') console.warn('reduxjs-toolkit-persist/getStoredState: Error in storage.getAllKeys');
        return reject(err);
      }
      ...
      keysToRestore.forEach(function (key) {
        storage.getItem(createStorageKey(key), function (err, serialized) {
          if (err && process.env.NODE_ENV !== 'production') console.warn('reduxjs-toolkit-persist/getStoredState: Error restoring data for key:', key, err);else restoredState[key] = rehydrate(key, serialized);
          completionCount += 1;
    ...
    function rehydrate(key, serialized) {
      ...
      try {
        ...
        if (process.env.NODE_ENV !== 'production') console.warn('reduxjs-toolkit-persist/getStoredState: Error restoring data for key:', key, err);
      }
